...
impl CPU {
    ...
    fn sbc(&mut self, mode: &AddressingMode) {
        ...
        self.set_flag_carry(sum > 0xFF);

        let res = sum as u8;
        self.set_flag_overflow(((self.reg_a ^ res) & (val ^ res) & 0x80) != 0);
        self.reg_a = res;
        self.update_zero_and_neg_flag(self.reg_a);
    }
    ...
    fn adc(&mut self, mode: &AddressingMode) {
        ...
        self.set_flag_carry(sum > 0xFF);
        let res = sum as u8;
        self.set_flag_overflow(((self.reg_a ^ res) & (data ^ res) & 0x80) != 0);
        self.reg_a = res;
        self.update_zero_and_neg_flag(self.reg_a);
    }
    ...
    fn plp(&mut self) {
        ...
        self.status = (pull_status & !0b0001_0000) | 0b0010_0000;
    }

    fn php(&mut self) {
        self.stk_push(self.status | 0b0011_0000);
    }
        ...
        loop {
            ...
            match opcode {
                ...
                0xB0 => {
                    let offset = self.mem_read(self.pc) as i8;
                    self.pc += 1;
                    let contains_carry = self.get_flag_carry();
                    if contains_carry {
                        self.pc = self.pc.wrapping_add(offset as u16);
                    }
                }
                ...
                0xF0 => {
                    let offset = self.mem_read(self.pc) as i8;
                    self.pc += 1;
                    let check_zero = (self.status & 0b0000_0010) != 0;
                    if check_zero {
                        self.pc = self.pc.wrapping_add(offset as u16);
                    }
                }
                ...
                0x30 => {
                    let offset = self.mem_read(self.pc) as i8;
                    self.pc += 1;
                    let check_neg = (self.status & 0b1000_0000) != 0;
                    if check_neg {
                        self.pc = self.pc.wrapping_add(offset as u16);
                    }
                }
